...
주제 전환 신호 (이어받지 않음):
- 새로운 *문서명* 또는 *비목/주제어*를 명시 ("매뉴얼 151p", "학생인건비는?", "FAQ Q3은?")
- 명백히 다른 주제어 ("그럼 회의비는?" 처럼 비교 의도가 있는 경우는 새 주제로 처리하되
  비교 대상이 직전 주제이면 doc_hint·키워드는 일부 보존 가능)
- 인사·감사 등 chat 신호

⚠️ *부분 전환* (조문·항·페이지만 바뀌고 문서는 직전과 동일):
- 사용자가 짧게 "11조", "제11조", "다음 조", "다음 페이지", "151p", "2항"만 입력
- 새 *문서명*이 명시되지 않았다면 직전 doc_name_hint 를 *반드시 이어받아* rewritten_query
  와 doc_name_hint 에 그대로 반영한다.
- 예: 직전 "혁신법 제12조 전문" → 현재 "11조"
       → doc_name_hint="법", target_articles=["제11조"], kind="article_lookup",
         rewritten_query="국가연구개발혁신법 제11조 전문"
- 예: 직전 "매뉴얼 151p Q1" → 현재 "152p"
       → doc_name_hint="매뉴얼", target_pages=[152], kind="page_lookup",
         rewritten_query="매뉴얼 152페이지 내용"
- 직전 doc_name_hint 가 비어 있으면 이어받기 불가 — 그대로 빈 값.
...
7) **rewritten_query** (멀티턴 핵심):
   ...
   - 예: 직전 "혁신법 제12조 전문" + 현재 "11조"
        → rewritten_query="국가연구개발혁신법 제11조 전문" (직전 문서명 그대로 흡수)
   - 예: 직전 "시행령 제20조 제1항" + 현재 "2항은?"
        → rewritten_query="국가연구개발혁신법 시행령 제20조 제2항"
